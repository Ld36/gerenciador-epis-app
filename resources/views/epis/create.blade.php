        ...
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('epis.store') }}">
                        @csrf
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="descricao" name="descricao" rows="3" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">{{ old('descricao') }}</textarea>
                            <x-input-error :messages="$errors->get('descricao')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="ca" :value="__('CA (Certificado de Aprovação)')" />
                            <x-text-input id="ca" class="block mt-1 w-full" type="text" name="ca" :value="old('ca')" required />
                            <x-input-error :messages="$errors->get('ca')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="validade_ca" :value="__('Validade do CA')" />
                            <x-text-input id="validade_ca" class="block mt-1 w-full" type="date" name="validade_ca" :value="old('validade_ca')" />
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="quantidade_estoque" class="block mt-1 w-full" type="number" name="quantidade_estoque" :value="old('quantidade_estoque', 0)" required min="0" />
