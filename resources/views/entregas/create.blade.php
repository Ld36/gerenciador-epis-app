<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Registrar Nova Entrega de EPI') }}
        </h2>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('entregas.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <select id="epi_id" name="epi_id" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" required>
                                ...
                                @foreach ($epis as $epi)
                                    <option value="{{ $epi->id }}" {{ old('epi_id') == $epi->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="quantidade_entregue" class="block mt-1 w-full" type="number" name="quantidade_entregue" :value="old('quantidade_entregue', 1)" required min="1" />
                            <x-input-error :messages="$errors->get('quantidade_entregue')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="data_entrega" :value="__('Data da Entrega')" />
                            <x-text-input id="data_entrega" class="block mt-1 w-full" type="date" name="data_entrega" :value="old('data_entrega', \Carbon\Carbon::now()->format('Y-m-d'))" required />
                            <x-input-error :messages="$errors->get('data_entrega')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="motivo" :value="__('Motivo da Entrega')" />
                            <textarea id="motivo" name="motivo" rows="3" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">{{ old('motivo') }}</textarea>
